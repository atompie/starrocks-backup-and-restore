...
- [x] 6.1 Run the full test suite (`pytest`) and confirm it passes. (719 passed excluding `tests/integration`; the one integration test, `test_full_backup_then_restore_recovers_dropped_database`, fails only on a pre-existing environment limitation — no real StarRocks/S3 endpoint available here, connection refused to `127.0.0.1:9000` — unrelated to this change.)
- [x] 6.2 Grep the repo (excluding `openspec/changes/archive/`) for every old path fragment (`/cluster/{cluster_id}/backups`, `/cluster/{cluster_id}/restores`, `/cluster/{cluster_id}/prunes`, `/cluster/{cluster_id}/schedules`, `/cluster/{cluster_id}/schedule/`, `/schedules/run-due`, `/cluster/{cluster_id}/repositories`, `/cluster/{cluster_id}/inventory-groups`) and confirm zero remaining references outside the archive. (Only matches are in this change's own `proposal.md`/`design.md`/`tasks.md`, which document the old→new mapping; no matches in code, tests, or docs.)
- [x] 6.3 Start the app and fetch `/openapi.json`; confirm every path listed matches the design.md table exactly, with no leftover old paths. (Verified via `create_app()` + `TestClient`; all 20 paths match the design.md table exactly, including the `manual-backups` tag rename.)
