...
The system SHALL allow an authenticated client to register a new StarRocks cluster by providing a unique name, host, port, user, and password, and SHALL persist the connection details (with the password encrypted at rest) in the metadata store. The system SHALL accept an optional `default_backend` value, which if provided MUST be one of the enabled backend identifiers `"thread"` or `"job"`, and SHALL default to `"thread"` when omitted.
...
- **WHEN** an authenticated client submits a cluster missing a required connection field (host, port, or user)
...
#### Scenario: Invalid default backend value is rejected
- **WHEN** an authenticated client submits a new cluster with a `default_backend` value other than `"thread"` or `"job"`
- **THEN** the system rejects the request with a 422 validation error and does not create the cluster
...
- **THEN** the system returns all registered clusters with their id, name, host, port, user, and `default_backend`, excluding password material
...
The system SHALL allow an authenticated client to update a registered cluster's connection fields (host, port, user, password) and its `default_backend`, re-encrypting a new password when provided. A submitted `default_backend` MUST be one of `"thread"` or `"job"`.
...
#### Scenario: Invalid default backend value is rejected on update
- **WHEN** an authenticated client submits a `default_backend` value other than `"thread"` or `"job"` when updating an existing cluster
- **THEN** the system rejects the request with a 422 validation error and does not modify the stored cluster record
...
- **THEN** the system attempts a real connection using the cluster's stored host, port, user, and password only, and returns a success result
