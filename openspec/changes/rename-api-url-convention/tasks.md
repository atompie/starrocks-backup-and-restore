...
- [ ] 6.1 Run the full test suite (`pytest`) and confirm it passes.
- [ ] 6.2 Grep the repo (excluding `openspec/changes/archive/`) for every old path fragment (`/cluster/{cluster_id}/backups`, `/cluster/{cluster_id}/restores`, `/cluster/{cluster_id}/prunes`, `/cluster/{cluster_id}/schedules`, `/cluster/{cluster_id}/schedule/`, `/schedules/run-due`, `/cluster/{cluster_id}/repositories`, `/cluster/{cluster_id}/inventory-groups`) and confirm zero remaining references outside the archive.
- [ ] 6.3 Start the app and fetch `/openapi.json`; confirm every path listed matches the design.md table exactly, with no leftover old paths.
